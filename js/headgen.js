var Head3D = (function () {
    ...
    function loadSkinImage(source, proxyUrl, skinProvider) {
        return new Promise(function (resolve, reject) {
            ...
            fetch(proxyUrl + "/api/minecraft/" + encodeURIComponent(source))
                ...
                .then(function (data) {
                    if (!data.skinUrl) {
                        throw new Error("Minecraft skin URL was not returned.");
                    }

                    image.src = data.skinUrl;
                })
                .catch(reject);
        });
    }
    ...
    function updateSettings(settings) {
        ...
        applyLayerScale(currentRender.subject, settingsData.layerScale);

        currentRender.lighting.ambient.intensity = settingsData.ambient;

        currentRender.lighting.hemisphere.intensity = settingsData.hemisphere;

        currentRender.lighting.directional.intensity = settingsData.directional;

        updateLightPosition(currentRender.lighting.directional, settingsData.lightX, settingsData.lightY, settingsData.lightZ);
